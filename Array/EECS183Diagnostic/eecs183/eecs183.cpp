...
void add(char arr[], int size, int val)
{
    ...
    for (int i = 0; i < size; i++)
    {
        ...
        while (character < MIN_PRINTABLE_ASCII)
            character += ASCII_PRINTABLE_RANGE;
        while (character > MAX_PRINTABLE_ASCII)
            character -= ASCII_PRINTABLE_RANGE;
...
{
    if (size1 <= 0 || size1 > SIZE || size2 <= 0 || size2 > SIZE || index1 + len - 1 > size1 || index2 + len - 1 > size2)
        return;
